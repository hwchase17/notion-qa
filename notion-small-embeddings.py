...
Each DB row is broken down meta data + title + single column. (i.e. 50 columns = 50 entries)
"""
...
row_template = '''Title information:
column-{{title-index}} name: {{title.prop.name}} 
column-{{title-index}} data type: {{title.prop.type}}  
column-{{title-index}} value: {{title.prop.value}}
...
class NotionRow:
    ...
    def serialize(self, database_name):
        ...
        #add title
        row_template_copy = row_template
        for i, prop in enumerate(self.properties):
            if prop.type == 'title' or prop.name == 'Name':
                row_template_copy = row_template_copy.replace('{{title-index}}', str(i))
                row_template_copy = row_template_copy.replace('{{title.prop.name}}', str(prop.name))
                row_template_copy = row_template_copy.replace('{{title.prop.type}}', str(prop.type))
                row_template_copy = row_template_copy.replace('{{title.prop.value}}', str(prop.value))
                # row_template_copy = row_template_copy.replace('{{title.name}}', str(prop.name))
                # row_template_copy = row_template_copy.replace('{{title.type}}', str(prop.type))
...
def get_rows(response, title):
    ...
    for i, row in enumerate(rows):
        ...
        extracted.extend(clean_row.serialize(title))
...
np.save('docs_np_moonchaser_xs_embedding_feb23.npy', docs_np) # save
np.save('metadata_np_moonchaser_xs_embedding_feb23.npy', metadatas_np) # save
